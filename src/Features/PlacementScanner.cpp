...
HUD_ELEMENT2_NO_DISABLE(pp_scan, HudType_InGame) {
	...
	case SetupState::SET_MATCH_POINT_B:
		...
		break;
	case SetupState::READY:
		status_text = "Placement scan is ready. Use pytal_pp_scan_set to begin.";
		break;
	case SetupState::RUNNING:
		status_text = "Scanning to pp_scan.tga... use pytal_pp_scan_set to cancel.";
		break;
	default:
		break;
	}
...
CON_COMMAND(pytal_pp_scan_set, "pytal_pp_scan_set - set the ppscan point where you're aiming.\n") {
	switch (g_setup.state) {
	case SetupState::NONE:
...
CON_COMMAND(pytal_pp_scan_reset, "pytal_pp_scan_reset - reset ppscan.\n") {
	g_setup.state = SetupState::NONE;
}
