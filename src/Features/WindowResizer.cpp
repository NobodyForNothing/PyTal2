...
Variable pytal_allow_resizing_window("pytal_allow_resizing_window", "0", 0, 1, "EXPERIMENTAL! Forces resizing on game's window.\n");
...
ON_EVENT(FRAME) {
	if (!pytal_allow_resizing_window.GetBool()) return;

#ifndef _WIN32
	console->Print("pytal_allow_resizing_window is currently working for Windows only.");
	pytal_allow_resizing_window.SetValue(0);
#endif
