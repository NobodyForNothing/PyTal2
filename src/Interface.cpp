...
void *Interface::GetPtr(const char *filename, const char *interfaceSymbol) {
	...
	if (!handle) {
		console->DevWarning("PYTAL: Failed to open module %s!\n", filename);
		return nullptr;
	}
	...
	if (!CreateInterface) {
		console->DevWarning("PYTAL: Failed to find symbol CreateInterface for %s!\n", filename);
		return nullptr;
	}
	...
	if (ret) {
		console->DevWarning("PYTAL: Failed to find interface with symbol %s in %s!\n", interfaceSymbol, filename);
		return nullptr;
	}
	return fn;
}
