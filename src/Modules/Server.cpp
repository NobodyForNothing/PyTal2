...
DETOUR_T(bool, Server::CheckJumpButton) {
	...
	if (server->AllowsMovementChanges()) {
		...
		if (pytal_autojump.GetBool() && !server->jumpedLastTime) {
		...
		jumped = (pytal_duckjump.isRegistered && pytal_duckjump.GetBool())
...
DETOUR(Server::PlayerMove) {
	...
	if (stat->jumps->isTracing && m_fFlags & FL_ONGROUND && m_MoveType != MOVETYPE_NOCLIP) {
		stat->jumps->EndTrace(server->GetAbsOrigin(player), pytal_stats_jumps_xy.GetBool());
	}
	...
	stat->velocity->Save(server->GetLocalVelocity(player), pytal_stats_velocity_peak_xy.GetBool());
	inspector->Record();
...
DETOUR(Server::PlayerRunCommand, CUserCmd *cmd, void *moveHelper) {
	if (!engine->IsGamePaused()) {
		if (pytal_tas_real_controller_debug.GetInt() == 3) {
...
static bool UTIL_FindClosestPassableSpace_Detour(const Vector &center, const Vector &extents, const Vector &ind_push, unsigned iterations, Vector &center_out, int axis_restriction_flags, FcpsTraceAdapter *trace_adapter) {
	if (pytal_fcps_override.GetBool() && sv_cheats.GetBool()) {
...
DETOUR(Server::ProcessMovement, void *player, CMoveData *move) {
	...
	// This is for pytal_trace_teleport_at
...
DETOUR_T(Vector*, Server::GetPlayerViewOffset, bool ducked) {
	...
	if (pytal_force_qc.GetBool() && sv_cheats.GetBool()) {
...
Variable pytal_always_transmit_heavy_ents("pytal_always_transmit_heavy_ents", "0", "Always transmit large but seldom changing edicts to clients to prevent lag spikes.\n");

extern Hook g_IsInPVS_Hook;
DETOUR_T(bool, Server::IsInPVS, void *info) {
	if (pytal_always_transmit_heavy_ents.GetBool()) {
...
DETOUR_B(Server::AirMove) {
	if (pytal_aircontrol.GetInt() >= 2 && server->AllowsMovementChanges()) {
...
Variable pytal_portals_thru_portals("pytal_portals_thru_portals", "0", "Allow firing portals through portals.\n");
// cvar callbacks dont want to fucking work so we'll just do this bs
ON_EVENT(PRE_TICK) {
	setAircontrol(server->AllowsMovementChanges() ? pytal_aircontrol.GetInt() : 0);
	setPortalsThruPortals(sv_cheats.GetBool() && pytal_portals_thru_portals.GetBool());
}
...
#endif
{
	...
	if (pytal_show_entinp.GetBool() && sv_cheats.GetBool()) {
...
DETOUR(Server::GameFrame, bool simulating)
{
	...
	if (pytal_tick_debug.GetInt() >= 3 || (pytal_tick_debug.GetInt() >= 2 && simulating)) {
...
DETOUR_T(void, Server::OnRemoveEntity, IHandleEntity *ent, CBaseHandle handle) {
	...
	if (!hasSerialChanged && pytal_prevent_ehm.GetBool()) {
...
#ifdef _WIN32
		if (pytal.game->Is(SourceGame_Portal2)) {
...
#else
	uintptr_t insn_addr = (uintptr_t)say_callback + (pytal.game->Is(SourceGame_EIPRelPIC) ? 55 : 88);
#endif
...
CON_COMMAND(pytal_coop_reset_progress, "pytal_coop_reset_progress - resets all coop progress\n") {
	if (engine->IsCoop()) {
		NetMessage::SendMsg(RESET_COOP_PROGRESS_MESSAGE_TYPE, nullptr, 0);
		resetCoopProgress();
	}
}
CON_COMMAND(pytal_give_fly, "pytal_give_fly [n] - gives the player in slot n (0 by default) preserved crouchfly.\n") {
	if (args.ArgC() > 2) return console->Print(pytal_give_fly.ThisPtr()->m_pszHelpString);
	if (!sv_cheats.GetBool()) return console->Print("pytal_give_fly requires sv_cheats.\n");
...
CON_COMMAND(pytal_give_betsrighter, "pytal_give_betsrighter [n] - gives the player in slot n (0 by default) betsrighter.\n") {
	if (args.ArgC() > 2) return console->Print(pytal_give_fly.ThisPtr()->m_pszHelpString);
	if (!sv_cheats.GetBool()) return console->Print("pytal_give_betsrighter requires sv_cheats.\n");
	int slot = args.ArgC() == 2 ? atoi(args[1]) : 0;
...
DETOUR_COMMAND(Server::say) {
	if (args.ArgC() != 2 || Utils::StartsWith(args[1], "!PYTAL:") || !networkManager.HandleGhostSay(args[1])) {
...
CON_COMMAND(pytal_paint_reseed, "pytal_paint_reseed <seed> - re-seed all paint sprayers in the map to the given value (-9999 to 9999 inclusive)\n") {
	if (args.ArgC() != 2) {
		console->Print(pytal_paint_reseed.ThisPtr()->m_pszHelpString);
		return;
	}
	...
	if (!sv_cheats.GetBool()) {
		console->Print("pytal_paint_reseed requires sv_cheats\n");
		return;
	}
