...
CON_COMMAND(pytal_font_get_name, "pytal_font_get_name <id> - gets the name of a font from its index\n") {
	if (args.ArgC() != 2) {
		return console->Print(pytal_font_get_name.ThisPtr()->m_pszHelpString);
	}
...
CON_COMMAND(pytal_font_list, "pytal_font_list - lists all available fonts\n") {
