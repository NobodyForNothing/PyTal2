...
Variable pytal_demo_overwrite_bak("pytal_demo_overwrite_bak", "0", 0, "Rename demos to (name)_bak if they would be overwritten by recording\n");
...
ON_EVENT(SESSION_END) {
	if (*engine->demorecorder->m_bRecording && pytal_autorecord.GetInt() == -1) {
...
DETOUR(EngineDemoRecorder::SetSignonState, int state) {
	...
	if (state == SIGNONSTATE_SPAWN) {
		if (engine->demorecorder->isRecordingDemo || *engine->demorecorder->m_bRecording || pytal_record_at_increment.GetBool()) {
	...
	if (state == SIGNONSTATE_FULL && needToRecordInitialVals) {
		...
		*/
		for (ConCommandBase *cmd = tier1->m_pConCommandList; cmd; cmd = cmd->m_pNext) {
			...
			if (Utils::StartsWith(cmd->m_pszName, "pytal_")) continue;
...
static std::string getDemoBakName(const char *base, int idx) {
	...
	if (idx == 1 && pytal_demo_overwrite_bak.GetInt() == 1) return Utils::ssprintf("%s/%s_bak.dem", engine->GetGameDirectory(), base);
...
static void preventOverwrite(const char *filename, int idx) {
	if (idx >= pytal_demo_overwrite_bak.GetInt()) return;
	try {
...
DETOUR(EngineDemoRecorder::StartRecording, const char *filename, bool continuously) {
	...
	if (pytal_demo_overwrite_bak.GetBool()) {
...
DETOUR(EngineDemoRecorder::StopRecording) {
	...
	if (engine->demorecorder->isRecordingDemo && pytal_autorecord.GetInt() == 1 && !engine->demorecorder->requestedStop) {
		*engine->demorecorder->m_nDemoNumber = engine->demorecorder->lastDemoNumber;
		*engine->demorecorder->m_bRecording = true;
	} else if (pytal_record_at_increment.GetBool()) {
		*engine->demorecorder->m_nDemoNumber = engine->demorecorder->lastDemoNumber;
...
Variable pytal_record_prefix("pytal_record_prefix", "", "A string to prepend to recorded demo names. Can include strftime format codes.\n", 0);
Variable pytal_record_mkdir("pytal_record_mkdir", "1", "Automatically create directories for demos if necesary.\n");
...
DETOUR_COMMAND(EngineDemoRecorder::record) {
	...
	if (args.ArgC() >= 2 && pytal_record_prefix.GetString()[0]) {
		time_t t = time(nullptr);
		struct tm *tm = localtime(&t);
		char *buf = new char[MAX_PATH + 1];
		size_t timelen = strftime(buf, MAX_PATH + 1, pytal_record_prefix.GetString(), tm);
		if (timelen) {
			strncat(buf, args[1], MAX_PATH - timelen);
			newArgs.m_ppArgv[1] = buf;
			prefixed = true;
		} else {
			console->Print("failed to add pytal_record_prefix\n");
			delete[] buf;
		}
	}
	...
	if (newArgs.ArgC() >= 2 && pytal_record_mkdir.GetBool() && !menu && !engine->demorecorder->isRecordingDemo) {
		try {
...
ON_EVENT(PRE_TICK) {
	...
	if (engine->demorecorder->m_bRecording) {
		if (event.simulating && !engine->demorecorder->hasNotified) {
			const char *cmd = "echo \"PYTAL " PYTAL_VERSION " (Built " PYTAL_BUILT ")\"";
...
ON_EVENT(PRE_TICK) {
	if (!engine->demoplayer->IsPlaying()) {
		if (pytal_record_at.GetInt() == -1) {
			engine->hasRecorded = true;  // We don't want to randomly start recording if the user sets pytal_record_at in this session
		} else if (!engine->hasRecorded && session->isRunning && event.tick >= pytal_record_at.GetInt()) {
			std::string cmd = std::string("record \"") + pytal_record_at_demo_name.GetString() + "\"";
...
ON_EVENT(CM_FLAGS) {
	if (engine->demorecorder->isRecordingDemo && event.end) {
		...
		Scheduler::InHostTicks(DEMO_AUTOSTOP_DELAY, [=]() {
			...
			if (pytal_challenge_autostop.GetInt() > 0) {
				...
				if (pytal_challenge_autostop.GetInt() == 2 || pytal_challenge_autostop.GetInt() == 3) {
					...
					if (pytal_challenge_autostop.GetInt() == 2) {
...
CON_COMMAND(pytal_stop, "pytal_stop <name> - stop recording the current demo and rename it to 'name' (not considering pytal_record_prefix)\n") {
	if (args.ArgC() != 2) return console->Print(pytal_stop.ThisPtr()->m_pszHelpString);
